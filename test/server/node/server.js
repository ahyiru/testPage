...
app.post('/auth/login', function(req, res) {
  User.findOne({ email: req.body.email }, function(err, user) {
    ...
    User.findOne({ password: req.body.password }, function(err, isMatch) {
      if (!isMatch) {
        return res.status(401).send({ message: 'Wrong password' });
      }
      res.send({ token: createJWT(user) });
...
/*
 |--------------------------------------------------------------------------
 | Login with weibo 20151201
 |--------------------------------------------------------------------------
 */
app.post('/auth/weibo', function(req, res) {
  var accessTokenUrl = 'https://api.weibo.com/oauth2/access_token';
  var peopleApiUrl = 'https://api.weibo.com/2/users/show.json';
  var params = {
    code: req.body.code,
